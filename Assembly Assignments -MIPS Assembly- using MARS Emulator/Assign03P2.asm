################################################################################
# Name:    Ryan Askew
# Class:   CS2318-001 (Assembly Language, Fall 2016)
...
#################
					addiu $sp, $sp, -109
					# clutter-reduction jump (string initialization)
					j StrInitCode		
endStrInit:					
#   do
#   {
...
####################(4)####################
					# pass arguments for ValidateInt
					addi $a0, $sp, 77
					add $a3, $sp, $0
					addi $a1, $0, 1
					addi $a2, $0, 7
...
####################(8)####################
					# doing the above commented line of C++ code
					addi $a0, $sp, 41 #entIntPrompt
					jal GetOneIntByVal
					
					lw $t4, 77($sp) #valsToDo
					sub $t3, $t4, $t1 #valsToDo - i
					sll $t3, $t3, 2
					
					addi $t5, $sp, 81 #array base addr
					add $t5, $t5, $t3 #intArr[valsToDo - i]
					
					sw $v0, 0($t5) #store value in intArr
###########################################
...
####################(7)####################
					# preparing/passing arguments to GetOneIntByAddr
					addi $a1, $sp, 41
					
					lw $t4, 77($sp) #valsToDo
					sub $a0, $t4, $t1 #valsToDo - i
					sll $a0, $a0, 2
					addi $t5, $sp, 81
					add $a0, $a0, $t5
...
####################(3)####################
					# doing the above line of commented C++ code
					add $t1, $0, $0
					lw $t5, 77($sp)
					addi $t2, $t5, -1
###########################################
...
####################(5)####################
					# preparing/passing args for SwapTwoInts
					addi $t0, $sp, 81 #intArr
					sll $t5, $t1, 2
					add $a0, $t0, $t5 #intArr + i
					sll $t5, $t2, 2
					add $a1, $t0, $t5 #intArr + j
###########################################
					jal SwapTwoInts
...
####################(3)####################
					#ppassing args for ShowIntArray
					addi $a0, $sp, 81
					addi $a2, $sp, 66
					lw $a1, 77($sp)
###########################################
...
####################(2)####################
					#passing arg for GetOneCharByAddr
					addi $a0, $sp, 56 #prompt
###########################################
...
GetOneIntByAddr:
#   cout << prompt;
					# $t0 has saved copy of $a0 as received
					move $t0, $a0		
					move $a0, $a1
					li $v0, 4
					syscall
...
# $a0: intArr + i
# $a1: intArr + j
#$t0: temporary for first int
#$t5: temporary for second int
#################
...
#}
					#prompt, then get a char, store in memory
					li $v0, 4
					syscall
